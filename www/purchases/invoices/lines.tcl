...
set class ::alt::PurchaseInvoiceLine
...
if {[info exists item_id] && [::xo::db::Class exists_in_db -id $item_id]} {
    ...
    if {!$confirmed_p} {
	set page_title "#altered.Edit_Purchase_Invoice_Line#"
    } else {
	set page_title "#altered.View_Purchase_Invoice_Line#"
	set mode view
    }
} else {
    if {!$confirmed_p} {
	set page_title "#altered.Create_Purchase_Invoice_Line#"
	set data [$class new -volatile]
    } else {
	set page_title "#altered.PurchaseInvoiceLines#"
    }
}
...
if {$show_form_p} {
    ...
    ad_form -name $form_name \
		...
		foreach var {description price vat_id unity_id} {
